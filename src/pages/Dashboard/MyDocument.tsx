...
import DocumentViewer from "../../components/dashboard/DocumentViewer";// ADD THIS IMPORT
import type { UploadedDocument } from "../../store/useDocumentStore";

const MyDocument: React.FC = () => {
  const [selectedDoc, setSelectedDoc] = useState<UploadedDocument | null>(null);
  const [modalOpen, setModalOpen] = useState(false);
  const [showDocumentViewer, setShowDocumentViewer] = useState(false); // ADD THIS
  ...
  const handleView = (doc: UploadedDocument) => {
    setSelectedDoc(doc);
    setShowDocumentViewer(true); // CHANGE THIS
  };
    ...
    <div className="flex-1 overflow-y-auto bg-dashboard-secondary dark:bg-background-dark/50 font-display">
      <div className="max-w-7xl mx-auto">
        ...
        {showDocumentViewer && selectedDoc ? (
          ...
          <>
            {/* Your existing header and table code */}
            ...
            {/* Your existing conditional rendering for documents */}
            {documents.length === 0 ? (
